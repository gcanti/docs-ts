...
function writeModules(M, modules) {
    return Array_1.array
        ...
        return getFile(module).chain(function (file) {
            return M.log("Printing module " + file.path).applySecond(M.writeFile(file.path, file.content));
        });
    })
...
function addModulesIndex(M) {
    var indexPath = path.join(outDir, 'modules', 'index.md');
    return M.exists(indexPath).chain(function (b) {
        return !b
            ? M.log("Adding modules/index.md to " + outDir + "...").chain(function () {
