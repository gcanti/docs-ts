...
function func(name, signature, description, since, location, deprecated, example) {
...
exports.func = func;
function method(name, signature, description, since, location, deprecated, example) {
    return { name: name, signature: signature, description: description, since: since, location: location, deprecated: deprecated, example: example };
}
exports.method = method;
function class_(name, signature, description, since, location, deprecated, example, methods) {
    return { name: name, signature: signature, description: description, since: since, location: location, deprecated: deprecated, example: example, methods: methods };
}
...
exports.index = index;
function module(path, interfaces, functions, classes) {
    return { type: 'Module', path: path, interfaces: interfaces, functions: functions, classes: classes };
}
...
function getInterfaces(sourceFile) {
    ...
    return Array_1.array.traverse(exports.monadValidation)(exportedInterfaceDeclarations, parseInterfaceDeclaration);
}
...
function parseVariableDeclaration(vd) {
    ...
    var _a = getAnnotationInfo(annotation), description = _a.description, since = _a.since, deprecated = _a.deprecated, example = _a.example;
    var signature = getFunctionVariableDeclarationSignature(vd);
    var name = vd.getName();
    return Validation_1.success({
        name: name,
        signature: signature,
...
function parseMethod(md) {
    ...
    return Validation_1.success(method(name, signature, description, since, getLocation(md), deprecated, example));
}
function parseClass(moduleName, c) {
    ...
    if (name === undefined) {
        ...
        return methods.map(function (methods) {
            return class_(name, signature_1, description_1, since_1, getLocation(c), deprecated_1, example_1, methods);
        });
    }
}
function getClasses(moduleName, sourceFile) {
    var exportedClasses = sourceFile.getClasses().filter(function (c) { return c.isExported(); });
    return Array_1.array.traverse(exports.monadValidation)(exportedClasses, function (cd) { return parseClass(moduleName, cd); });
}
exports.getClasses = getClasses;
function parse(file, source) {
    var moduleName = getModuleName(file.path);
    var sourceFile = getSourceFile(moduleName, source);
    return Apply_1.sequenceT(exports.monadValidation)(getInterfaces(sourceFile), getFunctions(moduleName, sourceFile), getClasses(moduleName, sourceFile)).map(function (_a) {
        var interfaces = _a[0], functions = _a[1], classes = _a[2];
        return module(file.path, interfaces, functions, classes);
    });
}
exports.parse = parse;
