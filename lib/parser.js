...
function documentable(name, description, since, location, deprecated, example) {
    return { name: name, description: description, since: since, location: location, deprecated: deprecated, example: example };
}
...
function parseInterfaceDeclaration(id) {
    ...
    return Validation_1.success(interface_(documentable(id.getName(), description, since, getLocation(id), deprecated, example), signature));
}
...
function parseFunctionDeclaration(moduleName, fd) {
    ...
    if (name === undefined || name.trim() === '') {
        ...
        return Validation_1.success(func(documentable(name, description, since, getLocation(fd), deprecated, example), signatures));
    }
}
function parseFunctionVariableDeclaration(vd) {
    ...
    var name = vd.getName();
    return Validation_1.success(func(documentable(name, description, since, getLocation(vd), deprecated, example), signatures));
}
...
function parseTypeAliasDeclaration(ta) {
    ...
    var name = ta.getName();
    return Validation_1.success(typeAlias(documentable(name, description, since, getLocation(ta), deprecated, example), signature));
}
...
function parseConstantVariableDeclaration(vd) {
    ...
    var name = vd.getName();
    return Validation_1.success(constant(documentable(name, description, since, getLocation(vd), deprecated, example), signature));
}
...
function parseMethod(md) {
    ...
    return Validation_1.success(method(documentable(name, description, since, getLocation(md), deprecated, example), signatures));
}
function parseClass(moduleName, c) {
    ...
    if (name === undefined) {
        ...
        return exports.monadParser.ap(methods.map(function (methods) { return function (staticMethods) {
            return class_(documentable(name, description_1, since_1, getLocation(c), deprecated_1, example_1), signature_1, methods, staticMethods);
        }; }), staticMethods);
    }
}
