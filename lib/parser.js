...
function module(path, description, interfaces, typeAliases, functions, classes, constants, exports) {
    return { path: path, description: description, interfaces: interfaces, typeAliases: typeAliases, functions: functions, classes: classes, constants: constants, exports: exports };
}
...
function parseExportDeclaration(ed) {
    ...
    var name = ed.getNamedExports()[0].getText();
    var comments = ed.getLeadingCommentRanges();
    if (comments.length > 0) {
        var text = comments[0].getText();
        var annotation = doctrine.parse(text, { unwrap: true });
        var _a = getAnnotationInfo(annotation), description = _a.description, since = _a.since, deprecated = _a.deprecated, example = _a.example;
        return Validation_1.success(export_(documentable(name, description, since, deprecated, example), signature));
    }
    else {
        return Validation_1.success(export_(documentable(name, Option_1.none, Option_1.none, false, Option_1.none), signature));
    }
}
...
function parse(path, source) {
    ...
    return sequenceS_1.sequenceS(exports.monadParser)({
        ...
        return module(path, getModuleDescription(sourceFile), interfaces, typeAliases, functions, classes, constants, exports);
    });
}
exports.parse = parse;
