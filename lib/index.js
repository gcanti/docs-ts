...
function getProjectName() {
    return Option_1.tryCatch(function () { return require('../package.json').name; });
}
function getExamples(nodes) {
    var projectName = getProjectName();
    function replaceProjectName(source) {
        return projectName.fold(source, function (projectName) {
    ...
    function toArray(prefix, x) {
        return x.example.foldL(function () { return Array_1.empty; }, function (source) {
            ...
            var code = (source.indexOf('assert.') !== -1 ? "import * as assert from 'assert'\n" : '') + replaceProjectName(source);
            return [function_1.tuple(name, code)];
        });
    }
