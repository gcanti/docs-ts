...
var PackageJSONDecoder = (0, function_1.pipe)(TD.struct({
    name: TD.string
}), TD.intersect(TD.partial({
    homepage: TD.string
})));
// -------------------------------------------------------------------------------------
// files
...
var readPackageJSON = (0, function_1.pipe)(RTE.ask(), RTE.chainTaskEitherK(function (_a) {
    var fileSystem = _a.fileSystem, logger = _a.logger;
    return (0, function_1.pipe)(fileSystem.readFile(path.join(process.cwd(), 'package.json')), TE.mapLeft(function () { return "Unable to read package.json in \"".concat(process.cwd(), "\""); }), TE.chainEitherK(function (json) {
        return (0, function_1.pipe)(json, Json.parse, E.mapLeft(function (u) { return E.toError(u).message; }));
    }), TE.flatMap(function (json) {
        return (0, function_1.pipe)(PackageJSONDecoder.decode(json), TE.mapLeft(function (decodeError) { return "Unable to decode package.json:\n".concat(TD.draw(decodeError)); }));
    }), TE.flatMap(function (_a) {
        var name = _a.name, homepage = _a.homepage;
        return (0, function_1.pipe)(logger.debug("Project name detected: ".concat(name)), TE.flatMap(function () {
            return (0, function_1.pipe)(O.fromNullable(homepage), TE.fromOption(function () { return "Missing homepage in package.json"; }), TE.map(function (homepage) { return ({ name: name, homepage: homepage }); }));
        }));
    }));
}));
...
var getDefaultSettings = function (projectName, projectHomepage) {
    return (0, function_1.pipe)(Config.build(projectName, projectHomepage), Config.resolveSettings);
};
...
var parseConfiguration = function (defaultSettings) {
    return function (file) {
        return (0, function_1.pipe)(RTE.ask(), RTE.chainTaskEitherK(function (_a) {
            var logger = _a.logger;
            return (0, function_1.pipe)((0, function_1.pipe)(Json.parse(file.content), E.mapLeft(toErrorMsg)), TE.fromEither, TE.tap(function () { return logger.info("Found configuration file"); }), TE.tap(function () { return logger.debug("Parsing configuration file found at: ".concat(file.path)); }), TE.flatMap(Config.decode), TE.bimap(function (decodeError) { return "Invalid configuration file detected:\n".concat(decodeError); }, function (settings) { return (__assign(__assign({}, defaultSettings), settings)); }));
        }));
    };
};
