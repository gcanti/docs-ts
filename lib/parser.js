...
function interface_(name, signature, description, since, location, deprecated, example) {
...
function constant(name, signature, description, since, location, deprecated, example) {
    return { name: name, signature: signature, description: description, since: since, location: location, deprecated: deprecated, example: example };
}
...
exports.index = index;
function module(path, interfaces, typeAliases, functions, classes, constants) {
    return { type: 'Module', path: path, interfaces: interfaces, typeAliases: typeAliases, functions: functions, classes: classes, constants: constants };
}
...
function parseInterfaceDeclaration(id) {
    ...
    return Validation_1.success(interface_(id.getName(), signature, description, since, getLocation(id), deprecated, example));
}
...
function getFunctions(moduleName, sourceFile) {
    ...
    var variableDeclarations = Array_1.array.traverse(exports.monadValidation)(exportedVariableDeclarations, parseFunctionVariableDeclaration);
...
function parseConstantVariableDeclaration(vd) {
    ...
    var name = vd.getName();
    return Validation_1.success(constant(name, signature, description, since, getLocation(vd), deprecated, example));
}
function getConstants(sourceFile) {
    ...
    return Array_1.array
        .traverse(exports.monadValidation)(exportedVariableDeclarations, parseConstantVariableDeclaration)
...
function parse(file, source) {
    ...
    return Apply_1.sequenceT(exports.monadValidation)(getInterfaces(sourceFile), getFunctions(moduleName, sourceFile), getTypeAliases(sourceFile), getClasses(moduleName, sourceFile), getConstants(sourceFile)).map(function (_a) {
        var interfaces = _a[0], functions = _a[1], typeAliases = _a[2], classes = _a[3], constants = _a[4];
        return module(file.path, interfaces, typeAliases, functions, classes, constants);
    });
}
exports.parse = parse;
