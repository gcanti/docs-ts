...
describe('FileSystem', () => {
  ...
  describe('utils', () => {
    ...
    it('readFile', async () => {
      ...
      const content = await _.readFile(file, 'utf8')()

      assert.deepStrictEqual(content, E.right('test1'))
    })
    ...
    it('writeFile', async () => {
      ...
      assert.deepStrictEqual(content, E.right('test2'))
    })
    ...
    it('exists', async () => {
      ...
      const exists = await _.exists(file)()

      assert.deepStrictEqual(exists, E.right(true))
    })
  ...
  describe('instances', () => {
    ...
    it('FileSystem', async () => {
      ...
      let exists = await _.FileSystem.exists(file4)()
      const content = await _.FileSystem.readFile(file4)()
      const found = await pipe(
      ...
      assert.deepStrictEqual(exists, E.right(true))
      assert.deepStrictEqual(content, E.right('4'))
      assert.deepStrictEqual(found, E.right(true))

      await _.FileSystem.remove(file4)()
      exists = await _.FileSystem.exists(file4)()

      assert.deepStrictEqual(exists, E.right(false))
    })
  })
})
