...
function checkExamples(examples, options) {
    var failures = check_1.check(examples, options);
...
function mangleExamples(examples, projectName) {
...
exports.mangleExamples = mangleExamples;
function main() {
    var projectName = require(path.join(process.cwd(), 'package.json')).name;
    var pattern = 'src/**/*.ts';
    var outDir = 'docs';
    var unParsedConfig = ts.readConfigFile('tsconfig.json', ts.sys.readFile).config;
    var options = ts.parseJsonConfigFileContent(unParsedConfig, ts.sys, process.cwd()).options;
    options.noEmit = true;
    ...
    function writeNode(node) {
        switch (node.type) {
            ...
            case 'Module':
                var header = markdown.printHeader(node.path.slice(1).join('/'), counter++);
    ...
    return parser.monadParser
        ...
        var examples = mangleExamples(getExamples(nodes), projectName);
        var check = doTypeCheckExamples ? checkExamples(examples, options) : Validation_1.success(undefined);
        return parser.monadParser.chain(check, function () {
            var sorted = Array_1.sort(Ord_1.contramap(function (node) { return node.path.join('/').toLowerCase(); }, Ord_1.ordString))(nodes);
            return Array_1.array.traverse(parser.monadParser)(sorted, writeNode);
        });
    })
        .map(function (a) {
