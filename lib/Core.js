...
var readSourcePaths = (0, function_1.pipe)(RTE.ask(), RTE.chainTaskEitherK(function (_a) {
    ...
    return (0, function_1.pipe)(fileSystem.search(path.join(config.srcDir, '**', '*.ts'), config.exclude), TE.map(RA.map(path.normalize)), TE.tap(function (paths) { return (0, function_1.pipe)(logger.info("".concat(paths.length, " module(s) found"))); }));
}));
...
var getConfigYML = (0, function_1.pipe)(RTE.ask(), RTE.chainTaskEitherK(function (_a) {
    ...
    return (0, function_1.pipe)(fileSystem.exists(filePath), TE.flatMap(function (exists) {
        return exists
            ? (0, function_1.pipe)(fileSystem.readFile(filePath), TE.map(function (content) { return (0, FileSystem_1.File)(filePath, resolveConfigYML(content, config), true); }))
            : TE.of((0, FileSystem_1.File)(filePath, "remote_theme: ".concat(config.theme, "\n\n# Enable or disable the site search\nsearch_enabled: ").concat(config.enableSearch, "\n\n# Aux links for the upper right navigation\naux_links:\n  '").concat(getHomepageNavigationHeader(config), "':\n    - '").concat(config.projectHomepage, "'"), false));
    }));
}));
...
var parseConfiguration = function (defaultConfig) {
...
var useDefaultConfig = function (defaultConfig) {
    return (0, function_1.pipe)(RTE.ask(), RTE.chainTaskEitherK(function (_a) {
        var logger = _a.logger;
        return (0, function_1.pipe)(logger.info('No configuration file detected, using default configuration'), TE.map(function () { return defaultConfig; }));
    }));
};
var getDocsConfiguration = function (projectName, projectHomepage) {
    return (0, function_1.pipe)(hasConfiguration, RTE.bindTo('hasConfig'), RTE.bind('defaultConfig', function () { return RTE.right(getDefaultConfig(projectName, projectHomepage)); }), RTE.flatMap(function (_a) {
        var defaultConfig = _a.defaultConfig, hasConfig = _a.hasConfig;
        return hasConfig
            ? (0, function_1.pipe)(readConfiguration, RTE.flatMap(parseConfiguration(defaultConfig)))
...
exports.main = (0, function_1.pipe)(RTE.ask(), RTE.flatMap(function (capabilities) {
    return (0, function_1.pipe)(readPackageJSON, RTE.flatMap(function (pkg) {
        return (0, function_1.pipe)(getDocsConfiguration(pkg.name, pkg.homepage), RTE.chainTaskEitherK(function (config) {
